...
# create the world
world = World(physics_dt=cfg['environment']['simulation_dt'], rendering_dt=cfg['environment']['rendering_dt'])
...
if not prim.IsValid():
    ...
    asset_path = (
        assets_root_path + env_cfg['usd_path'] if env_cfg['buildin'] is True else env_cfg['usd_path']
    )
...
if 'tables' in cfg.keys():
    ...
    if not prim.IsValid():
        ...
        asset_path = (
            assets_root_path + tables[0]['usd_path'] if tables[0]['buildin'] is True else tables[0]['usd_path']
        )
        prim.GetReferences().AddReference(asset_path)
        UsdGeom.Xformable(prim).AddTranslateOp().Set((tables[0]['position'][0], 
                                                      tables[0]['position'][1], 
                                                      tables[0]['position'][2]))
        
        UsdGeom.Xformable(prim).AddRotateXYZOp().Set((tables[0]['orientation'][0],
...
for camera in cameras:
    if camera['type'] == 'pointcloud':
        ...
        camera['name']+'_'+camera['type'], force=True, dtype="float32", shape=(camera['resolution'][1]* camera['resolution'][0], 3)
        )
        camera_pipes[camera['name']+'_'+camera['type']]=pipe
    elif camera['type'] == 'rgb':
        pipe = NumpyMemMapDataPipe(
        camera['name']+'_'+camera['type'], force=True, dtype="uint8", shape=(camera['resolution'][1], camera['resolution'][0], 4)
        )
        camera_pipes[camera['name']+'_'+camera['type']]=pipe
    elif camera['type'] == 'distance_to_camera':
        pipe = NumpyMemMapDataPipe(
        camera['name']+'_'+camera['type'], force=True, dtype="uint8", shape=(camera['resolution'][1], camera['resolution'][0])
        )
        camera_pipes[camera['name']+'_'+camera['type']]=pipe
...
meta_data = {
    ...
    'camera_intrinsics': [ann.getCameraIntrinsics(camera['name']) for camera in cameras],
    'camera_extrinsics': [ann.getCameraExtrinsics(camera['name']) for camera in cameras],
}
...
while simulation_app.is_running():
    ...
    if counter % (cfg['environment']['rendering_dt']//cfg['environment']['simulation_dt']) == 0:
        ...
        for camera in cameras:
            ...
            if camera['type'] == 'pointcloud':
                payload = data['data']
                if payload.shape[0]:
                    camera_pipes[camera['name']+'_'+camera['type']].write(np.zeros((camera['resolution'][1]* camera['resolution'][0], 3)), match_length=True)
                    camera_pipes[camera['name']+'_'+camera['type']].write(payload, match_length=True)
            else:
                if data.shape[0]:
                    camera_pipes[camera['name']+'_'+camera['type']].write(data, match_length=False)
